...
This example demonstrates performing of different actions with Google Drive files using GoogleDrive library.
...
public void execute() {
    ...
    Optional<GFile> uploadedFile = googleDrive.createFile("Test", new FileInputStream("test.txt"));
    ...
}
```
...
public void execute() {
    ...
    Optional<GFileInfo> sourceFile = googleDrive.getFileInfo(fileId);
    if (sourceFile.isPresent()) {
        googleDrive.renameFile(sourceFile.get(), "New file name");
    }
}
```
...
public void execute() {
    ...
    sourceFile<GFileInfo> sourceFile = googleDrive.getFileInfo(fileId);
    if (sourceFile.isPresent()) {

        GFileInfo targetFolder = googleDrive.createFolderIfAbsent("New target folder");

        googleDrive.moveFile(sourceFile.get(), targetFolder.getFileId());
    }
}
```
...
public void execute() {
    ...
    googleDrive.deleteFile(fileIdToDelete);
    ...
}
```
...
6. Run `main()` method of `DriveFilesManipulatingModule` class.

[down_git_link]: https://downgit.github.io/#/home?url=https://github.com/easy-rpa/openframework/tree/main/examples/google-drive/drive-files-manipulating
